...
@torch.no_grad()
def collect_l0_and_peak(model, tokenizer, n_max: int, peak_layer: int,
                        device, script: str):
...
def run_one_script(model, tokenizer, model_id: str, script: str,
                    n_max: int, device, n_layers: int) -> dict:
    ...
    if peak_layer == 0:
        ...
        return {
            "model":      model_id, "script": script,
            "n_max":      int(n_max), "peak_layer": 0, "n_layers": n_layers,
            "r2_l0":      None, "r2_peak": None, "rho": 1.0,
            "cka_full":   1.0, "cka_helix": 1.0,
            "trivial":    True,
        }

    H_l0, H_peak = collect_l0_and_peak(model, tokenizer, n_max, peak_layer,
    ...
    print(f"  {script:11} L={peak_layer:>3}  "
...
def main():
    ...
    ap.add_argument("--scripts", default="latin",
